...
public:
  ...
  inline int32_t GetBucket(::by_ref<TKey> key);
  ...
  inline int32_t TryAdd(::by_ref<TKey> key);

  /// @brief Method TryGetValue, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  template <typename TValue> inline bool TryGetValue(TKey key, ::by_ref<TValue> item);
