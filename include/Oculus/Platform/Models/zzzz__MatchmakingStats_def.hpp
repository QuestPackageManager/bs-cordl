...
public:
  ...
  __declspec(property(get = __get_DrawCount, put = __set_DrawCount)) uint32_t DrawCount;

  /// @brief Field LossCount, offset 0x14, size 0x4
  __declspec(property(get = __get_LossCount, put = __set_LossCount)) uint32_t LossCount;

  /// @brief Field SkillLevel, offset 0x18, size 0x4
  __declspec(property(get = __get_SkillLevel, put = __set_SkillLevel)) uint32_t SkillLevel;

  /// @brief Field SkillMean, offset 0x20, size 0x8
  __declspec(property(get = __get_SkillMean, put = __set_SkillMean)) double_t SkillMean;

  /// @brief Field SkillStandardDeviation, offset 0x28, size 0x8
  __declspec(property(get = __get_SkillStandardDeviation, put = __set_SkillStandardDeviation)) double_t SkillStandardDeviation;

  /// @brief Field WinCount, offset 0x30, size 0x4
  __declspec(property(get = __get_WinCount, put = __set_WinCount)) uint32_t WinCount;

  constexpr uint32_t& __get_DrawCount();

  constexpr uint32_t const& __get_DrawCount() const;

  constexpr void __set_DrawCount(uint32_t value);

  constexpr uint32_t& __get_LossCount();

  constexpr uint32_t const& __get_LossCount() const;

  constexpr void __set_LossCount(uint32_t value);

  constexpr uint32_t& __get_SkillLevel();

  constexpr uint32_t const& __get_SkillLevel() const;

  constexpr void __set_SkillLevel(uint32_t value);

  constexpr double_t& __get_SkillMean();

  constexpr double_t const& __get_SkillMean() const;

  constexpr void __set_SkillMean(double_t value);

  constexpr double_t& __get_SkillStandardDeviation();

  constexpr double_t const& __get_SkillStandardDeviation() const;

  constexpr void __set_SkillStandardDeviation(double_t value);

  constexpr uint32_t& __get_WinCount();

  constexpr uint32_t const& __get_WinCount() const;

  constexpr void __set_WinCount(uint32_t value);

  static inline ::Oculus::Platform::Models::MatchmakingStats* New_ctor(void* o);

  /// @brief Method .ctor addr 0x25b5b20 size 0xc8 virtual false final false
