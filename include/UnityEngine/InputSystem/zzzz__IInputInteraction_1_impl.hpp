#pragma once
// IWYU pragma private; include "UnityEngine/InputSystem/IInputInteraction_1.hpp"
#include "UnityEngine/InputSystem/zzzz__IInputInteraction_1_def.hpp"
/// @brief Convert operator to "Il2CppObject"
template <typename TValue>
  requires(::cordl_internals::value_type_constraint<TValue> && ::cordl_internals::default_constructor_constraint<TValue>)
constexpr UnityEngine::InputSystem::IInputInteraction_1<TValue>::operator Il2CppObject*() noexcept {
  return static_cast<Il2CppObject*>(static_cast<void*>(this));
}
/// @brief Convert to "Il2CppObject"
template <typename TValue>
  requires(::cordl_internals::value_type_constraint<TValue> && ::cordl_internals::default_constructor_constraint<TValue>)
constexpr Il2CppObject* UnityEngine::InputSystem::IInputInteraction_1<TValue>::i_Il2CppObject() noexcept {
  return static_cast<Il2CppObject*>(static_cast<void*>(this));
}
