#pragma once
// IWYU pragma private; include "UnityEngine\PlayMode.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(PlayMode)
// Forward declare root types
namespace UnityEngine {
struct PlayMode;
}
// Write type traits
MARK_VAL_T(::UnityEngine::PlayMode);
DEFINE_IL2CPP_CLASS(::UnityEngine::PlayMode, "UnityEngine", "PlayMode");
// Dependencies
namespace UnityEngine {
// Is value type: true
// CS Name: UnityEngine.PlayMode
struct CORDL_TYPE PlayMode {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __PlayMode_Unwrapped
  enum struct __PlayMode_Unwrapped : int32_t {
    __E_StopSameLayer = static_cast<int32_t>(0x0),
    __E_StopAll = static_cast<int32_t>(0x4),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __PlayMode_Unwrapped() const noexcept {
    return static_cast<__PlayMode_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr PlayMode();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr PlayMode(int32_t value__) noexcept;

  /// @brief Field StopAll value: I32(4)
  static ::UnityEngine::PlayMode const StopAll;

  /// @brief Field StopSameLayer value: I32(0)
  static ::UnityEngine::PlayMode const StopSameLayer;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 20200 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::UnityEngine::PlayMode, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::UnityEngine::PlayMode) == 0x4, "Size mismatch!");

} // namespace UnityEngine
