...
template <std::size_t offset> CORDL_HIDDEN constexpr inline void** getAtOffset(void* instance) {
...
#define CORDL_FIELD_NULL_CHECK(inst)                                                                                                                                                                   \
  if (!inst) throw ::cordl_internals::NullException(std::string("Field access on nullptr instance, please make sure your instance is not null"))
#else
...
template <il2cpp_utils::has_il2cpp_conversion T> constexpr inline void* convert(T&& inst) {
  return inst.convert();
}
} // namespace cordl_internals
} // namespace
