#pragma once
// IWYU pragma private; include "GlobalNamespace\MultiplayerSongEntitlementsStatus.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(MultiplayerSongEntitlementsStatus)
// Forward declare root types
namespace GlobalNamespace {
struct MultiplayerSongEntitlementsStatus;
}
// Write type traits
MARK_VAL_T(::GlobalNamespace::MultiplayerSongEntitlementsStatus);
DEFINE_IL2CPP_CLASS(::GlobalNamespace::MultiplayerSongEntitlementsStatus, "", "MultiplayerSongEntitlementsStatus");
// Dependencies
namespace GlobalNamespace {
// Is value type: true
// CS Name: MultiplayerSongEntitlementsStatus
struct CORDL_TYPE MultiplayerSongEntitlementsStatus {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __MultiplayerSongEntitlementsStatus_Unwrapped
  enum struct __MultiplayerSongEntitlementsStatus_Unwrapped : int32_t {
    __E_Ok = static_cast<int32_t>(0x0),
    __E_Invalid = static_cast<int32_t>(0x1),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __MultiplayerSongEntitlementsStatus_Unwrapped() const noexcept {
    return static_cast<__MultiplayerSongEntitlementsStatus_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr MultiplayerSongEntitlementsStatus();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr MultiplayerSongEntitlementsStatus(int32_t value__) noexcept;

  /// @brief Field Invalid value: I32(1)
  static ::GlobalNamespace::MultiplayerSongEntitlementsStatus const Invalid;

  /// @brief Field Ok value: I32(0)
  static ::GlobalNamespace::MultiplayerSongEntitlementsStatus const Ok;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 6190 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::GlobalNamespace::MultiplayerSongEntitlementsStatus, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::GlobalNamespace::MultiplayerSongEntitlementsStatus) == 0x4, "Size mismatch!");

} // namespace GlobalNamespace
