#pragma once
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cmath>
CORDL_MODULE_EXPORT(IBeatToTimeConvertor)
// Forward declare root types
namespace GlobalNamespace {
class IBeatToTimeConvertor;
}
// Write type traits
MARK_REF_PTR_T(::GlobalNamespace::IBeatToTimeConvertor);
// Type: ::IBeatToTimeConvertor
// SizeInfo { instance_size: 0, native_size: -1, calculated_instance_size: 16, calculated_native_size: 16, minimum_alignment: 8, natural_alignment: 0, packing: None, specified_packing: None }
namespace GlobalNamespace {
// Is value type: false
// Dependencies: []
// Self: TypeDefinitionIndex(TypeDefinitionIndex(4259))
// CS Name: ::IBeatToTimeConvertor*
class CORDL_TYPE IBeatToTimeConvertor {
public:
  // Declarations
  /// @brief Method ConvertBeatToTime, addr 0x0, size 0xffffffffffffffff, virtual true, abstract: true, final false
  inline float_t ConvertBeatToTime(float_t beat);

  // Ctor Parameters [CppParam { name: "", ty: "IBeatToTimeConvertor", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  IBeatToTimeConvertor(IBeatToTimeConvertor&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "IBeatToTimeConvertor", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  IBeatToTimeConvertor(IBeatToTimeConvertor const&) = delete;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
} // namespace GlobalNamespace
NEED_NO_BOX(::GlobalNamespace::IBeatToTimeConvertor);
DEFINE_IL2CPP_ARG_TYPE(::GlobalNamespace::IBeatToTimeConvertor*, "", "IBeatToTimeConvertor");
