#pragma once
// IWYU pragma private; include "GlobalNamespace/VRControllersRecorderExtensions.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "System/zzzz__Object_def.hpp"
CORDL_MODULE_EXPORT(VRControllersRecorderExtensions)
namespace GlobalNamespace {
struct PlaymodeOptions;
}
namespace GlobalNamespace {
struct VRControllersRecorder_Mode;
}
// Forward declare root types
namespace GlobalNamespace {
class VRControllersRecorderExtensions;
}
// Write type traits
MARK_REF_T(::GlobalNamespace::VRControllersRecorderExtensions*);
DEFINE_IL2CPP_CLASS(::GlobalNamespace::VRControllersRecorderExtensions*, "", "VRControllersRecorderExtensions");
// [Extension]
// Dependencies System.Object
namespace GlobalNamespace {
// Is value type: false
// CS Name: VRControllersRecorderExtensions
class CORDL_TYPE VRControllersRecorderExtensions : public ::System::Object {
public:
  // Declarations
  /// [Extension]
  /// @brief Method ToVRControllersRecorderMode, addr 0x58e12f8, size 0x18, virtual false, abstract: false, final false
  static inline ::GlobalNamespace::VRControllersRecorder_Mode ToVRControllersRecorderMode(::GlobalNamespace::PlaymodeOptions playmodeOptions);

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr VRControllersRecorderExtensions();

public:
  // Ctor Parameters [CppParam { name: "", ty: "VRControllersRecorderExtensions", modifiers: "&&", def_value: None, comment: None }]
  // @brief delete move ctor to prevent accidental deref moves
  VRControllersRecorderExtensions(VRControllersRecorderExtensions&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "VRControllersRecorderExtensions", modifiers: "const&", def_value: None, comment: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  VRControllersRecorderExtensions(VRControllersRecorderExtensionsconst&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 6522 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(sizeof(::GlobalNamespace::VRControllersRecorderExtensions) == 0x10, "Size mismatch!");

} // namespace GlobalNamespace
