#pragma once
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "System/zzzz__Object_def.hpp"
CORDL_MODULE_EXPORT(UnityAssetsHelper)
// Forward declare root types
namespace GlobalNamespace {
class UnityAssetsHelper;
}
// Write type traits
MARK_REF_PTR_T(::GlobalNamespace::UnityAssetsHelper);
// Type: ::UnityAssetsHelper
// SizeInfo { instance_size: 16, native_size: -1, calculated_instance_size: 16, calculated_native_size: 16, minimum_alignment: 8, natural_alignment: 0, packing: None, specified_packing: None }
namespace GlobalNamespace {
// Is value type: false
// Dependencies: {TypeDefinitionIndex(TypeDefinitionIndex(2611))}
// Self: TypeDefinitionIndex(TypeDefinitionIndex(13816))
// CS Name: ::UnityAssetsHelper*
class CORDL_TYPE UnityAssetsHelper : public ::System::Object {
public:
  // Declarations
  // Ctor Parameters [CppParam { name: "", ty: "UnityAssetsHelper", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  UnityAssetsHelper(UnityAssetsHelper&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "UnityAssetsHelper", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  UnityAssetsHelper(UnityAssetsHelper const&) = delete;

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr UnityAssetsHelper();

public:
  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(::cordl_internals::size_check_v<::GlobalNamespace::UnityAssetsHelper, 0x10>, "Size mismatch!");

} // namespace GlobalNamespace
NEED_NO_BOX(::GlobalNamespace::UnityAssetsHelper);
DEFINE_IL2CPP_ARG_TYPE(::GlobalNamespace::UnityAssetsHelper*, "", "UnityAssetsHelper");
