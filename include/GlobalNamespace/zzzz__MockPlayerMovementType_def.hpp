#pragma once
// IWYU pragma private; include "GlobalNamespace\MockPlayerMovementType.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(MockPlayerMovementType)
// Forward declare root types
namespace GlobalNamespace {
struct MockPlayerMovementType;
}
// Write type traits
MARK_VAL_T(::GlobalNamespace::MockPlayerMovementType);
DEFINE_IL2CPP_CLASS(::GlobalNamespace::MockPlayerMovementType, "", "MockPlayerMovementType");
// Dependencies
namespace GlobalNamespace {
// Is value type: true
// CS Name: MockPlayerMovementType
struct CORDL_TYPE MockPlayerMovementType {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __MockPlayerMovementType_Unwrapped
  enum struct __MockPlayerMovementType_Unwrapped : int32_t {
    __E_AI = static_cast<int32_t>(0x0),
    __E_MirrorPlayer = static_cast<int32_t>(0x1),
    __E_Recording = static_cast<int32_t>(0x2),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __MockPlayerMovementType_Unwrapped() const noexcept {
    return static_cast<__MockPlayerMovementType_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr MockPlayerMovementType();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr MockPlayerMovementType(int32_t value__) noexcept;

  /// @brief Field AI value: I32(0)
  static ::GlobalNamespace::MockPlayerMovementType const AI;

  /// @brief Field MirrorPlayer value: I32(1)
  static ::GlobalNamespace::MockPlayerMovementType const MirrorPlayer;

  /// @brief Field Recording value: I32(2)
  static ::GlobalNamespace::MockPlayerMovementType const Recording;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 6119 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::GlobalNamespace::MockPlayerMovementType, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::GlobalNamespace::MockPlayerMovementType) == 0x4, "Size mismatch!");

} // namespace GlobalNamespace
