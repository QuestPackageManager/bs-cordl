#pragma once
// IWYU pragma private; include "OSCE/Analytics/GetIdParams.hpp"
#include "OSCE/Analytics/zzzz__GetIdParams_def.hpp"
// Ctor Parameters [CppParam { name: "replace_rid", ty: "bool", modifiers: "", def_value: Some("{}"), comment: None }]
constexpr ::OSCE::Analytics::GetIdParams::GetIdParams(bool replace_rid) noexcept {
  this->replace_rid = replace_rid;
}
// Ctor Parameters []
constexpr ::OSCE::Analytics::GetIdParams::GetIdParams() {}
